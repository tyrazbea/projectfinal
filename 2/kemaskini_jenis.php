...
if(isset($_POST["a1"]))
{
	...
	$data = mysqli_query($connection,"UPDATE pakej set pakej='$a1' where id='$id'") or die(mysqli_error());
	echo "<script>alert('Kemaskini maklumat yang telah direkodkan');
	window.location = 'update_pakej.php'</script>";
}else
{
	$query = mysqli_query($connection,"SELECT * from pakej where id = '$id'");

	while ($result = mysqli_fetch_array($query)) 
	{
		$idPakej = $result['id'];
		$dataPakej = $result['pakej'];
	}
    ...
    <div class="col-md-4 col-md-offset-4">
      <form name="form1" action="kemaskini_jenis.php?id=<?php echo $id; ?>" method="POST">
        <fieldset>
          ...
          <!-- Form Name -->
          <legend><h3>KEMASKINI JENIS PAKEJ</h3></legend>

          <!-- Text input-->
          <div class="form-group">
            <label class="col-sm-2 control-label" for="textinput">Nama Pakej</label>
            <div class="col-sm-10">
              <input type="text" name="a1" id="a1" value="<?php echo $dataPakej; ?>" class="form-control">
            </div>
          </div>
